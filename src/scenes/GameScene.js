...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // ForestRenderer zwischen Hintergrund und Hauptbaum
    this.forestRenderer = new ForestRenderer(this, this.forest);

    this.tree.graphics.setDepth(5); // Hauptbaum oben
    ...
    this.time.addEvent({ delay: 1000, loop: true, callback: () => {
      ...
      if (this.forest.trees.length > 0) {
        this.resources.add({ symbiosis: this.forest.trees.length * 0.2 });
      }
  ...
  _checkGameOver() {
    if (this.mutations.getBonuses().immortal) return; // Weltenwurzel Stufe 3
  ...
  _showGameOver() {
    ...
    this.time.delayedCall(800, () => {
      ...
      this.add.text(W/2,H*0.50,'Der Baum ist gestorben.',{fontFamily:'"Cormorant Garamond",Georgia,serif',fontSize:'32px',fill:'#c87040',stroke:'#000',strokeThickness:3}).setOrigin(0.5).setDepth(51);
      this.add.text(W/2,H*0.60,'Jahr '+this.seasons.year+' \u2013 '+this.tree.phase.name+' \u2013 Wald: '+this.forest.trees.length+' B\u00e4ume',{fontFamily:'sans-serif',fontSize:'14px',fill:'#806050'}).setOrigin(0.5).setDepth(51);
      const btn=this.add.rectangle(W/2,H*0.72,220,42,0x2a1a0a,0.95).setInteractive({cursor:'pointer'}).setDepth(51).setStrokeStyle(1,0x6a3a1a);
      this.add.text(W/2,H*0.72,'\ud83c\udf31 Neu starten',{fontFamily:'sans-serif',fontSize:'15px',fill:'#d09060'}).setOrigin(0.5).setDepth(52);
      btn.on('pointerdown', () => { SaveSystem.deleteSave(); this.scene.restart({loadSave:false}); this._gameOverShown=false; });
    });
  }
  ...
  _drawBackground(season) {
    ...
    this.groundGfx.fillStyle(Phaser.Display.Color.GetColor(Math.min(255,gc.red+18),Math.min(255,gc.green+18),Math.min(255,gc.blue+8)),1);
    this.groundGfx.fillEllipse(W/2,H*0.78,W*1.5,80);
  }

  _buildStars() {
    this._stars = Array.from({length:90},()=>({x:Math.random(),y:Math.random()*0.65,r:0.5+Math.random()*1.2,phase:Math.random()*Math.PI*2,speed:0.5+Math.random()*1.5}));
  }
  ...
  _spawnParticle() {
    ...
    this.particleList.push({x:Math.random()*W,y:-10,vy:0.4+Math.random()*0.8,vx:(Math.random()-0.5)*0.7,size:1.5+Math.random()*3,alpha:0.5+Math.random()*0.4,color:colors[season]||0xffffff});
