...
export class UISystem {
  ...
  _buildHUD() {
    ...
    ['light','water','nutrients','symbiosis'].forEach((key, i) => {
      ...
      this.resBars[key]  = s.add.rectangle(rx, y+14, 0, 6, {light:0xf0d840,water:0x40a0f0,nutrients:0x70c030,symbiosis:0x40d0a0}[key]||0xffffff).setOrigin(0,0).setDepth(10);
    });
  ...
  _buildMonthStrip() {
    ...
    this._monthDots=[];
    const sx=W/2-99;
    for(let i=0;i<12;i++) this._monthDots.push(s.add.circle(sx+i*18,26,6,MC[i]).setDepth(10).setAlpha(0.35));
  }
  _updateMonthStrip() {
    ...
    this._monthDots.forEach((d,i)=>{ d.setAlpha(i<month?0.7:i===month?1.0:0.3); d.setScale(i===month?1.4:1.0); });
  }
  ...
  addEventLog(msg, type='info') {
    ...
    if(this._logTexts.length>=5){const old=this._logTexts.shift();s.tweens.add({targets:old,alpha:0,duration:300,onComplete:()=>old.destroy()});}
    const txt=s.add.text(W-265,170+this._logTexts.length*26,msg,{
      ...
      stroke:'#000',strokeThickness:1,wordWrap:{width:245},backgroundColor:'rgba(0,0,0,0.55)',padding:{x:6,y:3},
    }).setAlpha(0).setDepth(11);
    s.tweens.add({targets:txt,alpha:1,duration:300});
    s.tweens.add({targets:txt,alpha:0,delay:5000,duration:800,onComplete:()=>{txt.destroy();this._logTexts=this._logTexts.filter(t=>t!==txt);}});
  ...
  _renderPanel() {
    ...
    const panelBg=s.add.rectangle(PX,PY,PW,20+available.length*ROWH+16,0x080e08,0.95).setOrigin(0,0).setDepth(20).setStrokeStyle(1,0x3a6a2a,0.8);
    this._panelElements.push(panelBg);
    this._panelElements.push(s.add.text(PX+12,PY+8,'Aktive Mutationen: '+active+' / '+slots+' Slots',{fontFamily:'sans-serif',fontSize:'11px',fill:active>=slots?'#ff8060':'#70a050'}).setDepth(21));
    available.forEach((m,i)=>{
      const ry=PY+28+i*ROWH;
      const rowBg=s.add.rectangle(PX+6,ry,PW-12,ROWH-6,m.level>0?(m.level===3?0x1a3a0a:0x142a0e):0x0e110a,0.92).setOrigin(0,0).setDepth(21).setStrokeStyle(1,m.level===3?0x80ff40:m.level>0?0x50c030:m.type==='symbiosis'?0xc0a030:0x2a3a1a,0.7);
      this._panelElements.push(rowBg);
      const nc=m.level===3?'#a0ff60':m.level>0?'#70d040':m.unlocked?'#b0d080':'#505040';
      const nameTxt=s.add.text(PX+14,ry+6,m.emoji+' '+m.name,{fontFamily:'"Cormorant Garamond",Georgia,serif',fontSize:'13px',fill:nc}).setDepth(22);
      this._panelElements.push(nameTxt);
      for(let lvl=0;lvl<m.upgrades.length;lvl++){
        const dot=s.add.circle(PX+14+nameTxt.width+12+lvl*12,ry+12,4,lvl<m.level?0x70ff40:0x303020,lvl<m.level?1:0.5).setDepth(22);
        this._panelElements.push(dot);
      }
      this._panelElements.push(s.add.text(PX+PW-70,ry+8,'['+m.type+']',{fontFamily:'sans-serif',fontSize:'10px',fill:{passive:'#a0c880',active:'#80c0f0',symbiosis:'#f0c040',crisis:'#f06040'}[m.type]||'#888'}).setDepth(22));
      const sd=m.upgrades[m.level<m.upgrades.length?m.level:m.level-1];
      this._panelElements.push(s.add.text(PX+14,ry+26,sd.description,{fontFamily:'sans-serif',fontSize:'10px',fill:'#909880',wordWrap:{width:PW-40}}).setDepth(22));
      let stxt,scol;
      if(m.level===m.upgrades.length){stxt='\u2605\u2605\u2605 Maximum';scol='#a0ff60';}
      else if(m.active){const c=m.upgrades[m.level].cost;const cs=Object.entries(c).filter(([,v])=>v>0).map(([k,v])=>({light:'\u2600\ufe0f',water:'\ud83d\udca7',nutrients:'\ud83c\udf31'}[k]+v).join('  ');stxt='\u25b2 Stufe '+(m.level+1)+': '+(cs||'Kostenlos');scol='#c0d060';}
      else if(m.type==='crisis'&&!m.unlocked){stxt='\ud83d\udd12 Krise: '+m.requiredCrisis;scol='#806050';}
      else{const c=m.upgrades[0].cost;const cs=Object.entries(c).filter(([,v])=>v>0).map(([k,v])=>({light:'\u2600\ufe0f',water:'\ud83d\udca7',nutrients:'\ud83c\udf31'}[k]+v)).join('  ');stxt='Aktivieren: '+(cs||'Kostenlos');scol='#a0c860';}
      this._panelElements.push(s.add.text(PX+14,ry+56,stxt,{fontFamily:'sans-serif',fontSize:'10px',fill:scol}).setDepth(22));
      ...
      if(m.level<m.upgrades.length&&(m.type!=='crisis'||m.unlocked)&&!slotFull){
        ...
        const bb=s.add.rectangle(PX+PW-80,ry+34,72,22,0x1a3a1a,0.92).setOrigin(0,0).setDepth(22).setInteractive({cursor:'pointer'}).setStrokeStyle(1,0x4a8a3a);
        ...
        bb.on('pointerdown',()=>{
          ...
          if(r.ok){this.scene.tree.draw(this.scene.seasons.current.id,this.mutations.getVisuals());this._renderPanel();this.addEventLog('\ud83e\udde6 '+m.emoji+' '+m.name+(r.level===3?' (MAX!)':' (Stufe '+r.level+')'),'growth');}
        ...
        this._panelElements.push(s.add.text(PX+PW-74,ry+44,'\ud83d\udd12 Slot voll',{fontFamily:'sans-serif',fontSize:'9px',fill:'#806040'}).setDepth(22));
      }
    });
  }
  ...
  _renderForestPanel() {
    ...
    const rowsT = availTrees.length, rowsD = availDepths.length;
    const PH = 36 + rowsT*62 + 24 + rowsD*56 + 20;
    ...
    this._forestElements.push(
      s.add.text(PX+12,PY+10,'\ud83c\udf32 Wald  –  B\u00e4ume: '+this.forest.trees.length+' / '+slots+' Pl\u00e4tze',
        {fontFamily:'"Cormorant Garamond",Georgia,serif',fontSize:'14px',fill:'#c0d870'}).setDepth(21)
    );

    // Gepflanzte Bäume als kleine Tags
    ...
    availTrees.forEach((type,i)=>{
      ...
      const rowBg=s.add.rectangle(PX+6,ry,PW-12,56,canAfford?0x141e06:0x0c100a,0.9).setOrigin(0,0).setDepth(21).setStrokeStyle(1,canAfford?0x507030:0x283018,0.7);
      this._forestElements.push(rowBg);
      this._forestElements.push(
        ...
        s.add.text(PX+14,ry+36,
          Object.entries(type.cost).map(([k,v])=>({light:'\u2600\ufe0f',water:'\ud83d\udca7',nutrients:'\ud83c\udf31'}[k]+v)).join('  '),
          {fontFamily:'sans-serif',fontSize:'9px',fill:'#a09050'}).setDepth(22),
    ...
    availDepths.forEach((depth,i)=>{
      ...
      const rowBg=s.add.rectangle(PX+6,ry,PW-12,50,unlocked?0x080e14:0x09090a,0.9).setOrigin(0,0).setDepth(21).setStrokeStyle(1,unlocked?0x3060a0:0x202020,0.7);
      this._forestElements.push(rowBg);
      this._forestElements.push(
      ...
      if(!unlocked&&depth.unlockCost){
        this._forestElements.push(s.add.text(PX+14,ry+36,
          Object.entries(depth.unlockCost).map(([k,v])=>({light:'\u2600\ufe0f',water:'\ud83d\udca7',nutrients:'\ud83c\udf31'}[k]+v)).join('  '),
          {fontFamily:'sans-serif',fontSize:'9px',fill:'#706040'}).setDepth(22));
      }
      if(!unlocked&&(depth.unlockCost?canAfford:true)){
        ...
        const bStr=Object.entries(depth.passiveBonus).filter(([k,v])=>typeof v==='number'&&v>0).map(([k,v])=>({
          water:'\ud83d\udca7',nutrients:'\ud83c\udf31',allRatesBonus:'\u26a1',symbiosis:'\ud83e\udeb8',waterFloor:'Wasser min.'
        }[k]||k)+' +'+(v*100).toFixed(0)+'%').join('  ');
  ...
  _renderCodex() {
    ...
    cl.on('pointerdown',()=>this._closeCodex()); this._codexElements.push(cl);
    let curY=PY+40;
  ...
  update() {
    ...
    } else if(!nextP){ this.growthHint.setText('\ud83c\udf33 Urbaum \u2013 vollst\u00e4ndig'); }
    else { this.growthHint.setText('\ud83c\udf31 W\u00e4chst...'); }
