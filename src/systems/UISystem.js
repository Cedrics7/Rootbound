...
// Scrollbares Panel – Hilfsklasse
class ScrollPanel {
  constructor(scene, x, y, w, h, depth = 20) {
    this.scene = scene; this.x = x; this.y = y; this.w = w; this.h = h; this.depth = depth;
    this.scrollY = 0; this.contentH = 0;
    this._mask = null; this._zone = null;
    this._bg = scene.add.rectangle(x, y, w, h, 0x080e08, 0.97).setOrigin(0,0).setDepth(depth).setStrokeStyle(1,0x3a6a2a,0.8);
    // Maske für Clip
    ...
    this.container = scene.add.container(0, 0).setDepth(depth+1).setMask(this._mask);
    // Scroll-Zone
    this._zone = scene.add.zone(x, y, w, h).setOrigin(0,0).setDepth(depth+2).setInteractive();
    this._zone.on('wheel', (ptr, dx, dy) => {
      this.scrollY = Math.max(0, Math.min(this.contentH - h, this.scrollY + dy * 0.5));
      this.container.setPosition(0, -this.scrollY);
      this._updateScrollbar();
    });
    // Scrollbar
    this._sbTrack = scene.add.rectangle(x+w-8, y+2, 6, h-4, 0x202820, 0.7).setOrigin(0,0).setDepth(depth+3);
    this._sbThumb = scene.add.rectangle(x+w-8, y+2, 6, 40, 0x5a9a4a, 0.85).setOrigin(0,0).setDepth(depth+4);
  }
  setContentHeight(h) {
    this.contentH = h;
    this._updateScrollbar();
  ...
  _updateScrollbar() {
    const ratio = Math.min(1, this.h / Math.max(1, this.contentH));
    const thumbH = Math.max(20, this.h * ratio);
    const maxScroll = Math.max(1, this.contentH - this.h);
    const thumbY = this.y + 2 + (this.scrollY / maxScroll) * (this.h - thumbH - 4);
    this._sbThumb.setPosition(this.x + this.w - 8, thumbY);
    this._sbThumb.height = thumbH;
    this._sbThumb.setVisible(ratio < 1);
    this._sbTrack.setVisible(ratio < 1);
  }
  add(el) { this.container.add(el); return el; }
  destroy() {
    this.container.destroy();
    this._bg.destroy();
    this._zone.destroy();
    this._sbTrack.destroy();
    this._sbThumb.destroy();
    this.scene.make?.graphics && this._mask?.destroy?.();
  }
}
...
export class UISystem {
  constructor(scene, resources, seasons, tree, mutations, codex, forest) {
    ...
    this._forestScroll    = null;
    this._codexScroll     = null;
    this._buildHUD();
  }
  ...
  _clearPanel() {
    if (this._scrollPanel) { this._scrollPanel.destroy(); this._scrollPanel = null; }
  ...
  _renderPanel() {
    ...
    const maxPanelH = H - PY - 20;
    const contentH  = 28 + available.length * ROWH + 8;

    // Scrollbares Panel
    const sp = new ScrollPanel(s, PX, PY, PW, Math.min(maxPanelH, contentH), 20);
    sp.setContentHeight(contentH);
    this._scrollPanel = sp;
    this._panelElements.push(
      sp.add(s.add.text(PX+12, PY+8, 'Aktive Mutationen: '+active+' / '+slots+' Slots', {
    ...
    available.forEach((m, i) => {
      ...
      const rowBg = sp.add(s.add.rectangle(PX+6, ry, PW-12, ROWH-6,
      ...
      const nameTxt = sp.add(s.add.text(PX+14, ry+6, m.emoji+' '+m.name, {
        fontFamily:'"Cormorant Garamond",Georgia,serif', fontSize:'13px', fill:nc,
      }).setDepth(22));

      for (let lvl = 0; lvl < m.upgrades.length; lvl++) {
        sp.add(s.add.circle(PX+14+nameTxt.width+12+lvl*12, ry+12, 4,
          lvl<m.level ? 0x70ff40 : 0x303020, lvl<m.level ? 1 : 0.5
        ).setDepth(22));
      }

      sp.add(s.add.text(PX+PW-70, ry+8, '['+m.type+']', {
      ...
      sp.add(s.add.text(PX+14, ry+26, sd.description, {
      ...
      sp.add(s.add.text(PX+14, ry+56, stxt, { fontFamily:'sans-serif', fontSize:'10px', fill:scol }).setDepth(22));
      ...
      if (m.level<m.upgrades.length && (m.type!=='crisis'||m.unlocked) && !slotFull) {
        ...
        const bb = sp.add(s.add.rectangle(PX+PW-80, ry+34, 72, 22, 0x1a3a1a, 0.92)
          .setOrigin(0,0).setDepth(22).setInteractive({ cursor:'pointer' }).setStrokeStyle(1,0x4a8a3a));
        const bt = sp.add(s.add.text(PX+PW-44, ry+45, bl, { fontFamily:'sans-serif', fontSize:'9px', fill:'#80d060' }).setOrigin(0.5).setDepth(23));
        ...
        sp.add(s.add.text(PX+PW-74, ry+44, '🔒 Slot voll', { fontFamily:'sans-serif', fontSize:'9px', fill:'#806040' }).setDepth(22));
      }
    });
    sp.setContentHeight(contentH);
  }

  // ── Forest Panel (scrollbar) ─────────────────────────────────────────────
  _openForestPanel()  { this.forestPanelOpen=true;  this._renderForestPanel(); }
  _closeForestPanel() {
    this.forestPanelOpen=false;
    if (this._forestScroll) { this._forestScroll.destroy(); this._forestScroll=null; }
    for (const el of this._forestElements) el.destroy();
    this._forestElements = [];
  }
  ...
  _renderForestPanel() {
    ...
    const sp = new ScrollPanel(s, PX, PY, PW, Math.min(maxH, contentH), 20);
    sp.setContentHeight(contentH);
    this._forestScroll = sp;

    const push = (el) => { this._forestElements.push(el); return sp.add(el); };
    push(s.add.text(PX+12, PY+10, '🌲 Wald  –  Bäume: '+this.forest.trees.length+' / '+slots+' Plätze', {
    ...
    if (this.forest.trees.length>0) {
      ...
      push(s.add.text(PX+12, PY+28, tagTxt, { fontFamily:'sans-serif', fontSize:'10px', fill:'#809060' }).setDepth(21));
    }
    ...
    availTrees.forEach((type, i) => {
      ...
      push(s.add.text(PX+14, ry+36, Object.entries(type.cost).map(([k,v])=>(EMOJI_MAP[k]||k)+v).join('  '), { fontFamily:'sans-serif', fontSize:'9px', fill:'#a09050' }).setDepth(22));
      ...
      if (this.forest.trees.length<slots && canAfford) {
        ...
        const bt = push(s.add.text(PX+PW-46, ry+31, 'Pflanzen', { fontFamily:'sans-serif', fontSize:'10px', fill:'#90d040' }).setOrigin(0.5).setDepth(23));
    ...
    push(s.add.text(PX+12, depY, '🗓️ Wurzeltiefe', { fontFamily:'"Cormorant Garamond",Georgia,serif', fontSize:'13px', fill:'#a08860' }).setDepth(21));
    ...
    availDepths.forEach((depth, i) => {
      ...
      if (!unlocked && depth.unlockCost) push(s.add.text(PX+14, ry+36, Object.entries(depth.unlockCost).map(([k,v])=>(EMOJI_MAP[k]||k)+v).join('  '), { fontFamily:'sans-serif', fontSize:'9px', fill:'#706040' }).setDepth(22));
      if (!unlocked && (depth.unlockCost ? canAfford : true)) {
        ...
        const bt = push(s.add.text(PX+PW-46, ry+25, 'Erkunden', { fontFamily:'sans-serif', fontSize:'10px', fill:'#60a0d0' }).setOrigin(0.5).setDepth(23));
        ...
        const bStr = Object.entries(depth.passiveBonus).filter(([,v])=>typeof v==='number'&&v>0).map(([k,v])=>(DEPTH_EMOJI[k]||k)+' +'+(v*100).toFixed(0)+'%').join('  ');
  ...
  _closeCodex() {
    this.codexOpen=false;
    if (this._codexScroll) { this._codexScroll.destroy(); this._codexScroll=null; }
  ...
  _renderCodex() {
    ...
    const sp = new ScrollPanel(s, PX, PY, PW, Math.min(maxH, contentH), 30);
    sp.setContentHeight(contentH);
    this._codexScroll = sp;
    const push = (el) => { this._codexElements.push(el); return sp.add(el); };

    push(s.add.text(PX+16, PY+12, '📖 Ökosystem-Codex', { fontFamily:'"Cormorant Garamond",Georgia,serif', fontSize:'18px', fill:'#a0d8a0' }).setDepth(31));
    push(s.add.text(PX+PW-16, PY+16, this.codex.getUnlocked().length+' / '+this.codex.getAll().length+' entdeckt', { fontFamily:'sans-serif', fontSize:'11px', fill:'#60a070' }).setOrigin(1,0).setDepth(31));
    const cl = push(s.add.text(PX+PW-12, PY+8, '✕', { fontFamily:'sans-serif', fontSize:'14px', fill:'#808080' }).setOrigin(1,0).setDepth(32).setInteractive({ cursor:'pointer' }));
    cl.on('pointerdown', () => this._closeCodex());

    let curY = PY+40;
    for (const cat of cats) {
      push(s.add.text(PX+16, curY, cat, { fontFamily:'"Cormorant Garamond",Georgia,serif', fontSize:'13px', fill:'#7a9870' }).setDepth(31));
      curY += 20;
  ...
  update() {
    ...
    if (nextP && !this.tree.isGrowing) {
      ...
      const symReq = nextP.requiredSymbioses||0, symCur = this.mutations.getActiveSymbioses();
      let allMet = true;
