...
export class CreatureUISystem {
  constructor(scene, creatureSystem, onArchetypeChosen, geneticSummary = null) {
    ...
    this._elements      = [];
    this._questPanel    = [];
    this._crisisEls     = [];   // Notfall-Banner
    this._built         = false;
    this._questBtn      = null;
  ...
  showArchetypeChoice() {
    ...
    // Genetisches Gedächtnis anzeigen
    if (this._geneticSummary && this._geneticSummary.runs > 0) {
  ...
  offerCrisisQuest(questDef) {
    ...
    // Pulsierender roter Rahmen + Banner
    ...
    const txt = push(
      s.add.text(BX + BW/2, BY + BH/2,
        '🆘 ' + questDef.emoji + ' ' + questDef.name,
        { fontFamily: 'sans-serif', fontSize: '10px', fill: '#ff8060' }
      ).setOrigin(0.5).setDepth(14)
    );
    // Puls-Tween auf den Rahmen
    s.tweens.add({
      targets: bg, alpha: { from: 0.96, to: 0.60 },
      yoyo: true, repeat: -1, duration: 600,
    });

    // Annehmen-Button (erscheint 300ms nach Banner)
    s.time.delayedCall(300, () => {
      if (!this._crisisOffered) return;
      const btn = push(
        s.add.rectangle(BX + BW - 46, BY + BH/2, 82, 20, 0x5a1000, 0.96)
          .setOrigin(0.5).setDepth(14).setInteractive({ cursor: 'pointer' })
    ...
    // Schutz-Indikator unter dem XP-Balken
    const shield = push(
