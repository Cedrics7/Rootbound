...
/**
 * CreatureUISystem – Archetyp-Wahl-Screen, Quest-Panel, Inventar, XP-Bar.
 */
export class CreatureUISystem {
  ...
  showArchetypeChoice() {
    ...
    // Verdunkelung
    const ov = s.add.rectangle(W / 2, H / 2, W, H, 0x000000, 0.75).setDepth(40);
    this._elements.push(ov);

    this._elements.push(
      s.add.text(W / 2, H * 0.12, '🌿 Rootbound', {
        fontFamily: '"Cormorant Garamond",Georgia,serif', fontSize: '28px', fill: '#a0d878',
      }).setOrigin(0.5).setDepth(41),
      s.add.text(W / 2, H * 0.21, 'Du erwachst in einem leeren Wald.\nWer bist du?', {
    ...
    CREATURE_ARCHETYPES.forEach((arch, i) => {
      ...
      const card = s.add.rectangle(cx, cy, cardW, cardH, 0x0a1a0a, 0.93)
        .setOrigin(0, 0.5).setDepth(41).setStrokeStyle(2, 0x3a6a2a)
        .setInteractive({ cursor: 'pointer' });

      s.add.text(cx + cardW / 2, cy - 58, arch.emoji, { fontSize: '36px' }).setOrigin(0.5).setDepth(42);
      s.add.text(cx + cardW / 2, cy - 14, arch.name, {
        fontFamily: '"Cormorant Garamond",Georgia,serif', fontSize: '16px', fill: '#c0d870',
      }).setOrigin(0.5).setDepth(42);
      s.add.text(cx + cardW / 2, cy + 12, arch.description, {
        fontFamily: 'sans-serif', fontSize: '10px', fill: '#909878',
        wordWrap: { width: cardW - 16 }, align: 'center',
      }).setOrigin(0.5).setDepth(42);

      const btn = s.add.rectangle(cx + cardW / 2, cy + 66, cardW - 20, 26, 0x1a3a1a, 0.95)
        .setOrigin(0.5).setDepth(42).setInteractive({ cursor: 'pointer' }).setStrokeStyle(1, 0x4a8a3a);
      s.add.text(cx + cardW / 2, cy + 66, 'Wählen', {
      ...
      this._elements.push(card, btn);
    });
  }

  // ── HUD aufbauen (nach Archetyp-Wahl) ─────────────────────────────────────
  buildHUD() {
    if (this._built) return;
    this._built = true;
    const s = this.scene, W = s.scale.width;

    // Quest-Button (oben links, unter den Haupt-Buttons)
    const bg = s.add.rectangle(16, 214, 170, 26, 0x0a140a, 0.90)
      .setOrigin(0, 0).setInteractive({ cursor: 'pointer' }).setDepth(11).setStrokeStyle(1, 0x3a5a1a);
    const lbl = s.add.text(101, 227, this.creature.archetype.emoji + ' Quest starten  ▼', {
    ...
    // XP-Bar (unter Quest-Button)
    s.add.text(16, 248, 'Tier Lv.', { fontFamily: 'sans-serif', fontSize: '10px', fill: '#607850' }).setDepth(11);
    this._levelText = s.add.text(56, 248, '1', { fontFamily: 'sans-serif', fontSize: '10px', fill: '#a0d060' }).setDepth(11);
    s.add.rectangle(16, 260, 140, 6, 0x1a1a1a).setOrigin(0, 0).setDepth(11);
    this._xpBar  = s.add.rectangle(16, 260, 0, 6, 0xa0ff60).setOrigin(0, 0).setDepth(11);
    this._xpText = s.add.text(160, 258, '', { fontFamily: 'sans-serif', fontSize: '9px', fill: '#607850' }).setDepth(11);

    // Inventar-Label
    this._invLabel = s.add.text(16, 272, '', { fontFamily: 'sans-serif', fontSize: '9px', fill: '#708060', wordWrap: { width: 170 } }).setDepth(11);

    this._elements.push(bg, lbl, this._xpBar, this._xpText, this._levelText, this._invLabel);
    this.updateHUD();
  }
  ...
  updateHUD() {
    ...
    const invStr = c.inventory.length
      ? c.inventory.map(i => i.emoji).join(' ')
      : 'Kein Inventar';
    this._invLabel.setText(invStr);

    // Quest-Button Label aktualisieren
    if (this._questBtn) {
      if (c.isOnQuest()) {
  ...
  _renderQuestPanel() {
    ...
    quests.forEach((q, i) => {
      ...
      this._questPanel.push(row, name, desc, dur);
    });
  }
